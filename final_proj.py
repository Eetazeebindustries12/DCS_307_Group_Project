...
class Location():
    ...
    def __repr__(self):
        '''if self._user is None:
            self._fill = " "
            return self._fill
        if self._user._infected:
            if self._user._susceptible:
                self._fill = "3"
            else:
                self._fill = "2"
        else:
...
#######################################################################################
    ...
    def block(self,other:'User'):
        """_summary_: interaction type that should occasionally occur between an infected or not-infected
            users, this should remove the infected user from the simulation.

            analogous to effective report on social media 
    ...
    def question(self):
        """_summary_
            this is a non interactive action that users can take if they are not able to interact during a round of the simulation
            not sure this is
        """
...
#######################################################################################
    ...
    def cure(self,other:'User'):
        """_summary_
            this is the interaction between two users that removes susceptibility,
            could potentially be avaiable to both infected and uninfected users, or maybe just useable by uninfected users
        Args:
            other (User): _description_
        """
        t = rng.random(1)
